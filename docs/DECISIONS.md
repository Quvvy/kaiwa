...
27. Prompt assembled per turn from prefs (`persona.build_tutor_system_prompt`); `max_sentences` enforces short replies.
...
47. No multi-user profiles / accounts.
...
51. Phase 4 focuses on **daily-use polish** (first-run, startup helper, TTS error clarity) and **learning feedback** (practice from chat/memory weaknesses) — not new product platforms.
